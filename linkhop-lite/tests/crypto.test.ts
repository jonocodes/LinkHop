...
describe("deriveEncryptionKey", () => {
  it("derives a CryptoKey from password", async () => {
    const key = await deriveEncryptionKey("test-password");
    expect(key).toBeDefined();
    expect(key.type).toBe("secret");
    expect(key.algorithm).toMatchObject({ name: "AES-GCM", length: 256 });
  });

  it("same password produces equivalent keys", async () => {
    const key1 = await deriveEncryptionKey("same-password");
    const key2 = await deriveEncryptionKey("same-password");
  ...
  it("different passwords produce different keys", async () => {
    const key1 = await deriveEncryptionKey("password-a");
    const key2 = await deriveEncryptionKey("password-b");
...
describe("encryptBody / decryptBody", () => {
  it("round-trips plaintext through encrypt then decrypt", async () => {
    const key = await deriveEncryptionKey("round-trip");
  ...
  it("produces different ciphertext each time (random IV)", async () => {
    const key = await deriveEncryptionKey("iv-test");
  ...
  it("returns null for corrupted ciphertext", async () => {
    const key = await deriveEncryptionKey("corrupt-test");
    const result = await decryptBody(key, "not-valid-base64!!", "AAAAAAAAAAAAAAAA");
    expect(result).toBeNull();
  });

  it("returns null for wrong key", async () => {
    const keyA = await deriveEncryptionKey("key-a");
    const keyB = await deriveEncryptionKey("key-b");
