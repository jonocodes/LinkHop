...
Create a short-lived 6-digit PIN that a new device can use to register. The PIN is single-use and expires after 10 minutes. Any previous unused PINs for the requesting device are deleted.

**Endpoint:** `POST /api/pairings/pin`
...
Register a new device using a pairing PIN to obtain an authentication token.

**Endpoint:** `POST /api/pairings/pin/register`

**Authentication:** None (requires pairing PIN)
...
**Parameters:**
- `pin` (required): 6-digit pairing PIN generated by an existing device or admin
...
**Error Responses:**
- `400 Bad Request`: Invalid/expired PIN or device name conflict
...
Fetch a single message by ID. Only the recipient device may retrieve it.
...
| `POST /api/pairings/pin/register` | 10 requests | per hour per IP |
...
| `invalid_pairing_pin` | 400 | Pairing PIN invalid or expired |
